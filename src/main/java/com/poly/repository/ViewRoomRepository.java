...
public interface ViewRoomRepository extends JpaRepository<ViewRoom, Integer> {
	 @Query("SELECT COALESCE(SUM(v.visitCount), 0) FROM ViewRoom v WHERE v.room.id = :roomId")
    ...
    @Modifying
    @Transactional
    @Query("UPDATE ViewRoom v SET v.visitCount = v.visitCount + 1, v.visitDate = CURRENT_TIMESTAMP WHERE v.room.Id = :roomId AND v.userId = :userId")
    void incrementVisitCount(@Param("roomId") int roomId, @Param("userId") Integer userId);

    @Query("SELECT v FROM ViewRoom v WHERE v.room.Id = :roomId AND v.userId = :userId")
